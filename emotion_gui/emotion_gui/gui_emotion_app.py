...
#model = tf.keras.models.load_model(r"C:\\Users\\caleb\\OneDrive\\Documentos\\emotion_gui\\emotion_gui\\fer_pro_model_final.keras")
#model = tf.keras.models.load_model(r"fer_pro_model_final.keras")
#import os

#ruta_modelo = os.path.join(os.path.dirname(__file__), "fer_pro_model_final.keras")
#print("Ruta completa del modelo:", ruta_modelo)
...
def mostrar_splash_con_gif():
    ...
    def cerrar_splash():
        if splash.winfo_exists():
            splash.destroy()
            iniciar()  # Llama directo a iniciar tu app

    splash.after(4000, cerrar_splash)
    splash.mainloop()
''' def cerrar_splash():
...
ventana_inicio.title("Bienvenido")
ventana_inicio.geometry("800x400")
#ventana_inicio.configure(bg="#1e1e2f")  # Fondo oscuro
ventana_inicio.resizable(False, False)
...
def iniciar():
    ventana_inicio.destroy()  # Cierra la ventana actual
    subprocess.Popen(["python", "app.py"])
